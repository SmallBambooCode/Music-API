...
function getRuntimeAllowlistState() {
  return {
    ...
    runtimeOnly: true,
    note: 'This switch is stored in serverless runtime memory only and may reset after redeploy, cold start, region switch, or instance recycle.',
  };
}
...
async function checkAllowlist(req, pathname = '') {
  ...
  if (isAdminPath(pathname)) {
    return {
      ...
      rules: info.rules,
      candidates,
      matched: { kind: 'admin_path', value: 'admin endpoints bypass source allowlist and require ADMIN_PASSWORD' },
      source: 'admin_bypass',
      corsOrigin: '*',
      runtimeSwitch: getRuntimeAllowlistState(),
    };
