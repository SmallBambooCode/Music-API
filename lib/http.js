...
function sendJson(req, res, status, payload, corsOrigin = '*') {
  ...
  res.setHeader('Cache-Control', status >= 400 ? 'no-store' : 'public, max-age=20');
...
function redirect(req, res, location, status = 302, corsOrigin = '*') {
  setCors(res, corsOrigin);
  res.statusCode = status;
...
function getPath(req) {
  try { return new URL(req.url, getOrigin(req)).pathname; }
  catch { return '/api'; }
}
...
module.exports = { setCors, sendJson, sendText, redirect, getOrigin, getPath, getQuery, apiBaseUrl, sendMaybeJsonp, readBody, readJson };
