...
async function httpGet(url, referer = REFERER) {
...
async function fetchSongInfo(hash, userId = '', token = '') {
...
async function resolvePlayUrlTracker(hash, userId = '', token = '') {
...
async function fetchLyrics(hash, songName, duration) {
  ...
  const dlBody = await httpGet(dlUrl, '');
  if (!dlBody) return '';
  try {
...
// 获取播放 URL (方案1 → 方案2)
async function songUrl(hash, userId = '', token = '') {
  const info = await fetchSongInfo(hash, userId, token);
  if (info && info.url) return { ok: true, url: info.url, name: info.name, singer: info.singer, time: info.time };
  const trackerUrl = await resolvePlayUrlTracker(hash, userId, token);
...
async function lyric(hash) {
  const info = await fetchSongInfo(hash);
  return fetchLyrics(hash, info ? info.name : '', info ? info.time : 0);
}

// 一次性返回完整单曲信息 (加速插件调用, 服务端并发)
async function songFull(hash, userId = '', token = '') {
  const [urlResult, lyricResult] = await Promise.all([
    songUrl(hash, userId, token),
    lyric(hash),
  ]);
  return {
    id: hash,
