...
async function resolvePlayUrlTracker(hash, userId = '', token = '', cookie = '') {
  ...
  // v0.27.1: 3 种参数组合并发执行 (原串行最坏 15s, 现最坏 5s)
  const attempts = [
    // 方案A: 带 VIP 标志 + area_id (移动APP VIP播放)
    `http://trackercdn.kugou.com/i/v2/?key=${key}&hash=${hash}&br=hq&appid=1005&pid=2&behavior=play&cmd=25&filename=${hash}.mp3&vip=1&area_id=1${uid ? `&userid=${uid}` : ''}${tk ? `&token=${tk}` : ''}`,
    // 方案B: 标准 trackercdn (免费歌曲)
    `http://trackercdn.kugou.com/i/v2/?key=${key}&hash=${hash}&br=hq&appid=1005&pid=2&behavior=play&cmd=25&filename=${hash}.mp3${uid ? `&userid=${uid}` : ''}${tk ? `&token=${tk}` : ''}`,
    // 方案C: br=sq (标准质量, 有些VIP歌曲允许sq播放)
    `http://trackercdn.kugou.com/i/v2/?key=${key}&hash=${hash}&br=sq&appid=1005&pid=2&behavior=play&cmd=25&filename=${hash}.mp3${uid ? `&userid=${uid}` : ''}${tk ? `&token=${tk}` : ''}`,
  ];

  const bodies = await Promise.all(attempts.map(u => httpGet(u, '', cookie)));
...
// 获取播放 URL
// v0.27.1: 改为全并发 (原 fetchSongInfo → www → tracker 串行, 最坏 15s; 现最坏 5s)
async function songUrl(hash, userId = '', token = '', cookie = '') {
  // 并发执行: 歌曲信息 + wwwapi VIP + trackercdn 兜底
  const [info, wwwUrl, trackerUrl] = await Promise.all([
    fetchSongInfo(hash, userId, token, cookie),
    resolvePlayUrlWWW(hash, userId, token, cookie),
    resolvePlayUrlTracker(hash, userId, token, cookie),
  ]);
  const name = info ? info.name : '未知';
  const singer = info ? info.singer : '未知';
  const time = info ? info.time : 0;
  // 优先级: info.url (免费歌曲) > wwwUrl (VIP) > trackerUrl (兜底)
  const url = (info && info.url) || wwwUrl || trackerUrl || '';
  if (url) return { ok: true, url, name, singer, time };
  return { ok: false, url: '', name, singer, time };
}

// 获取歌词 (单独端点)
// v0.27.1: 并发执行 fetchSongInfo + fetchLyrics (原串行最坏 10s, 现最坏 5s)
async function lyric(hash, cookie = '') {
  // fetchLyrics 需要 songName 做关键词搜索, 但可以用 hash 兜底 (lyrics.kugou.com 支持 hash 查询)
  // 并发: 歌曲信息 + 直接用 hash 查歌词 (不依赖 songName)
  const [info, lyricByHash] = await Promise.all([
    fetchSongInfo(hash, '', '', cookie),
    fetchLyrics(hash, '', 0, cookie),
  ]);
  // 如果用 hash 直接查到了歌词, 优先返回
  if (lyricByHash) return lyricByHash;
  // 兜底: 用 songName 重新查 (info 有 songName 时)
  if (info && info.name) {
    return fetchLyrics(hash, info.name, info.time, cookie);
  }
  return '';
}
