...
async function qrLoginCheck(token) {
  ...
  try {
    // ptqrlogin: 查询登录状态 (用 fetch, 只需要 body)
    ...
    let httpStatus = 0;
    try {
      const resp = await fetch(ptqrloginUrl, {
        headers: {
          'User-Agent': UA,
          'Referer': REFERER_QR_XLOGIN,
          'Cookie': `qrsig=${sess.qrsig}; pt_login_sig=${sess.ptLoginSig}`,
          'Accept': '*/*',
        },
...
// v2.4.3: 重试 2 次超时 3 秒, 总最长时间 3+0.4+3+0.4+3=9.8 秒 (避免 Vercel 10 秒超时)
// 支持登录态:
//   - uin (QQ 号) + qqmusic_key (QQ 音乐鉴权 token, F12 复制)
//   - cookie: 完整 document.cookie 字符串 (v2.4.0 新增, 优先级高于 uin+qqmusicKey)
async function musicuRequest(payload, retries = 2, uin = '', qqmusicKey = '', cookie = '') {
  ...
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      clearTimeout(timeout);
      if (!resp.ok) { if (i < retries) await new Promise(r => setTimeout(r, 400 * (i + 1))); continue; }
...
// cookie: 完整 document.cookie (v2.4.0, 优先级更高)
