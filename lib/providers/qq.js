...
async function qrLoginCheck(token) {
  ...
  try {
    ...
    // 注意: QQ 返回的参数间可能有空格, 且登录成功时 URL 用反引号 ` 包裹
    // 如: ptuiCB('66','0','','0','二维码未失效。', '')
    //     ptuiCB('0','0','`https://ptlogin2.qzone.qq.com/check_sig?...`','0','登录成功！','nick')
    const cleanText = text.replace(/`/g, '');
    const m = cleanText.match(/ptuiCB\(\s*'(\d+)'\s*,\s*'(\d+)'\s*,\s*'([^']*)'\s*,\s*'(\d+)'\s*,\s*'([^']*)'\s*,\s*'([^']*)'\s*\)/);
    if (!m) {
