...
// VIP 解灰的音源优先级
// 注意: 不含 bilibili/bilivideo (B站已移除)
// 注意: 不含 kuwo (防盗链返回"暂时无法播放"TTS语音, 即使加Referer也无法播放)
// 优先使用可直接播放的源: migu/pyncmd/kugou
...
// 验证 URL 是否可播放 (HEAD 请求检查 Content-Type 和 Content-Length)
// 解灰源可能返回防盗链 URL (如 kuwo 返回TTS语音), 需要验证
async function isUrlPlayable(url) {
  try {
    ...
    // 音频类型 (audio/* 或 octet-stream) 且大小 > 100KB (排除TTS语音约16KB)
    return (contentType.includes('audio') || contentType.includes('octet-stream') || contentType.includes('mpeg'))
...
// 跨平台解灰: 用歌名+歌手构建 data, 逐个尝试每个源并验证可播放性
// 带超时保护 (Vercel 30s 限制, 每个源最多 10s)
async function crossUnblock(songmid, name, singer, duration) {
  ...
  // 逐个尝试每个源, 验证 URL 可播放性
  for (const source of unblockSources()) {
    try {
      const timeout = new Promise(r => setTimeout(() => r(null), 10000));
      const result = await Promise.race([
        unmMatch(`qq_${songmid}`, [source], data),
        timeout,
      ]);
      if (result && typeof result.url === 'string' && result.url) {
        // 验证 URL 可播放性 (排除防盗链返回的TTS语音)
        if (await isUrlPlayable(result.url)) {
          return { url: result.url, source: `unblock:${result.source || source}` };
        }
      }
    } catch (_) {}
  }
