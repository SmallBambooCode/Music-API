...
// 从 cookie 字符串中提取酷我鉴权信息
// 支持两种 cookie 格式:
//   1. 浏览器 F12 复制的完整 cookie (含 userid/websid/sid/Hm_Iuvt csrf)
//   2. 短信登录返回的 kw_user_id/kw_token
function extractKuwoAuth(cookie) {
  if (!cookie) return { userId: '', token: '', websid: '', sid: '', csrf: '' };
  // userid (浏览器 cookie) 或 kw_user_id (短信登录)
  const uidMatch = cookie.match(/(?:^|;\s*)userid=([^;]+)/i)
    || cookie.match(/(?:^|;\s*)kw_user_id=([^;]+)/i);
  // csrf: Hm_Iuvt_cdb524f42f23cer9b268564v7y735ewrq2324 (浏览器) 或 kw_token (短信登录)
  const csrfMatch = cookie.match(/(?:^|;\s*)Hm_Iuvt_cdb524f42f23cer9b268564v7y735ewrq2324=([^;]+)/i)
    || cookie.match(/(?:^|;\s*)kw_token=([^;]+)/i);
  // websid, sid (浏览器会话)
  const websidMatch = cookie.match(/(?:^|;\s*)websid=([^;]+)/i);
  const sidMatch = cookie.match(/(?:^|;\s*)sid=([^;]+)/i);
  return {
    userId: uidMatch ? uidMatch[1] : '',
    token: csrfMatch ? csrfMatch[1] : '',
    csrf: csrfMatch ? csrfMatch[1] : '',
    websid: websidMatch ? websidMatch[1] : '',
...
// 获取播放 URL
// 方案1: www.kuwo.cn/api/v1/www/music/playUrl (VIP, 需要 csrf header + cookie)
// 方案2: antiserver (免费歌曲兜底, 返回纯文本 URL)
async function songUrl(mid, userId = '', token = '', cookie = '') {
  const auth = extractKuwoAuth(cookie);
  const uid = userId || auth.userId;
  const csrf = auth.csrf || token;
  // 构造完整 cookie (优先用透传的完整 cookie, 否则从提取的 fields 拼)
  let finalCookie = cookie;
  if (!finalCookie) {
    const parts = [];
    if (uid) parts.push(`userid=${uid}`);
    if (auth.websid) parts.push(`websid=${auth.websid}`);
    if (auth.sid) parts.push(`sid=${auth.sid}`);
    if (csrf) parts.push(`Hm_Iuvt_cdb524f42f23cer9b268564v7y735ewrq2324=${csrf}`);
    finalCookie = parts.join('; ');
  }
  // 方案1: www VIP 接口 (需要 csrf header)
  if (uid && csrf) {
    try {
      const controller = new AbortController();
      const timeout = setTimeout(() => controller.abort(), 5000);
      const vipUrl = `https://www.kuwo.cn/api/v1/www/music/playUrl?mid=${mid}&type=music&br=320kmp3&httpsStatus=1`;
      const headers = {
        'User-Agent': UA,
        'Referer': REFERER_WWW,
        'Accept': 'application/json, text/plain, */*',
      ...
      const resp = await fetch(vipUrl, { headers, signal: controller.signal });
      clearTimeout(timeout);
      if (resp.ok) {
        const body = await resp.text();
        const root = JSON.parse(body);
        if (root.code === 200 && root.data && root.data.url) {
          console.log(`[Kuwo VIP] 获取成功: ${root.data.url.slice(0, 80)}...`);
          return { ok: true, url: root.data.url, time: 0 };
        }
        console.log(`[Kuwo VIP] code=${root.code}, msg=${root.msg || ''}`);
      }
    } catch (e) {
      console.log(`[Kuwo VIP] 异常: ${e.message}`);
    }
  }
  // 方案2: antiserver (免费歌曲兜底)
  ...
  if (!body) return { ok: false, url: '', time: 0 };
...
// ==================== 已移除: QR 扫码登录 + 短信登录 ====================
// v0.23.0: 移除酷我扫码登录和短信登录
// 原因: 服务器 IP 无法获取 VIP 歌曲播放 URL
//   - www.kuwo.cn/api/v1/www/music/playUrl 返回 "The request is illegal!" (反爬虫)
//   - 浏览器 cookie 中的 userid/websid 绑定浏览器 IP, 服务器用无效
//   - 短信登录返回的 kw_token/kw_user_id 也无法获取 VIP URL
