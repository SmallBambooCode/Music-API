...
// 酷我账号密码登录 (供 app.js 调用)
// POST https://wapi.kuwo.cn/api/www/login/loginByKw
// body: { username, password, verifyCode, verifyCodeToken, userIp }
// 返回: { ok, userId?, token?, nickname?, cookie?, message? }
async function loginByKw(username, password, verifyCode = '', verifyCodeToken = '', userIp = '') {
  try {
    const crypto = require('node:crypto');
    // 先访问首页拿 csrf (cookie Hm_Iuvt_cdb524f42f23cer9b268564v7y735ewrq2324)
    const homeResp = await fetch('https://www.kuwo.cn/', {
      headers: { 'User-Agent': UA, Accept: 'text/html' },
    });
    let csrf = '';
    const setCookie = homeResp.headers.get('set-cookie') || '';
    const csrfMatch = setCookie.match(/Hm_Iuvt_cdb524f42f23cer9b268564v7y735ewrq2324=([^;]+)/);
    if (csrfMatch) csrf = csrfMatch[1];

    const loginUrl = 'https://wapi.kuwo.cn/api/www/login/loginByKw';
    const body = {
      username: String(username || ''),
      password: crypto.createHash('md5').update(String(password || ''), 'utf8').digest('hex'),
      verifyCode,
      verifyCodeToken,
      userIp,
    };
    ...
    if (!text) return { ok: false, message: '酷我登录: 服务器无响应' };
    let root;
    try { root = JSON.parse(text); } catch { return { ok: false, message: '酷我登录: 响应解析失败' }; }
    if (root.code !== 200) {
      return { ok: false, message: `酷我登录失败: ${root.msg || root.message || '账号或密码错误'}` };
    }
    const cookies = root.data && root.data.cookies;
    if (!cookies) return { ok: false, message: '酷我登录: 未返回 cookie' };
    // cookies 是 {key: val} 对象, 转成 cookie 字符串
    const cookieParts = [];
    let userId = '';
    let token = '';
    let nickname = '';
    ...
    console.error('[Kuwo loginByKw] 异常:', err.message);
    return { ok: false, message: `酷我登录异常: ${err.message}` };
  }
}
