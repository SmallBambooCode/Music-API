...
// 多平台 API 路由层 v0.28.0
// 支持: netease / kugou / kuwo (QQ provider 已移除, 限流超时不可用)
// 已彻底移除所有解灰 (B站/kuwo/migu/kugou/pyncmd/bodian 等源全部废弃)
// 已彻底移除所有用户登录/绑定功能 (VIP 登录无法在服务器端实现)
// 仅支持免费歌曲播放; 网易云若服务端配置 NCM_COOKIE 可直连 VIP 歌曲
//
// 端点:
...
//   /api?server={server}&type=url&id=xxx     — 播放 URL (仅免费歌曲)
//   /api?server={server}&type=lrc&id=xxx     — 歌词文本
//   /api?server={server}&type=song_full&id=xxx — 完整单曲 (name+url+lyric+time)
...
async function handleApi(req, res, reqUrl) {
  ...
  // ==================== 扫码登录路由 (v0.28.0: 已移除登录功能, 统一返回 not_supported) ====================
  if (action === 'qr_create') {
    return json(res, 200, { ok: false, error: 'not_supported', message: '扫码登录已移除 (VIP 登录无法在服务器端实现)' });
  }
  ...
  // v0.28.0: 已移除用户登录功能, 所有平台都不传 userId/token/cookie
  // 网易云通过服务端 NCM_COOKIE 环境变量访问 VIP 歌曲 (若已配置)
  ...
  try {
    ...
    if (type === 'url') {
      ...
      if (!result.ok) {
        ...
        return json(res, 200, {
          ...
          message: server === 'netease' ? '网易云未返回可播放 URL (可能是 VIP 歌曲)' :
                   '可能是 VIP 歌曲, 请切换其他源',
          server,
          needsLogin: false,
        });
    ...
    if (server === 'netease' && ['song', 'playlist', 'album', 'artist'].includes(type)) {
      return json(res, 200, await meting.resolve(
        type,
        id,
        meting.originFromReq(req),
        {
          limit,
          offset,
          includeMeta: bool(reqUrl.searchParams.get('meta')),
        }
      ));
    }
