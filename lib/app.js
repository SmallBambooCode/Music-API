...
function shouldBypassAllowlist(action, pathname) {
  ...
  if (action === 'health' || action === 'admin-status' || action === 'admin-toggle' || action === 'qr_bind' || action === 'manual_bind' || action === 'kw_login') return true;
  return false;
}
...
async function handleApi(req, res, reqUrl) {
  ...
  // ==================== 酷我账号密码登录端点 ====================
  // 酷我网页版无扫码登录, 用账号密码登录获取 cookie
  // 参数: username, password
  if (action === 'kw_login') {
    const username = (reqUrl.searchParams.get('username') || '').trim();
    const password = (reqUrl.searchParams.get('password') || '').trim();
    if (!username || !password) {
      return json(res, 400, { ok: false, error: 'missing_fields', message: '请输入酷我账号和密码' });
    }
    const kuwoProvider = providers.get('kuwo');
    if (!kuwoProvider || !kuwoProvider.loginByKw) {
      return json(res, 400, { ok: false, error: 'kw_login_not_supported', message: '酷我账号密码登录不可用' });
    }
    const result = await kuwoProvider.loginByKw(username, password);
    if (!result.ok) {
