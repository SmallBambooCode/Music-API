...
// VIP 歌曲获取方案:
//   - QQ音乐: 浏览器登录后复制 Cookie (uin + qqmusic_key), vkey 接口可获取 VIP purl
//   - 网易云: 服务端扫码登录, 登录态可获取 VIP 歌曲
//   - 酷狗/酷我: 服务器 IP 无法获取 VIP URL (token 绑浏览器 IP), 改用网易云兜底
//
// 端点:
...
// v0.23.0: 移除 kugou/kuwo 分支 (服务器无法获取 VIP URL, 改用网易云兜底)
function buildLoginInfo(platform, result) {
  if (platform === 'qq') {
    const uin = result.uin || result.userId || '';
    const pSkey = result.pSkey || result.token || '';
    // 如果用户提供了完整 cookie (含 qqmusic_key), 优先使用
    const cookie = result.cookie || `uin=o0${uin}; qqmusic_key=${pSkey}`;
    // 从 cookie 中提取 qqmusic_key 作为 token (vkey 接口需要)
    let qqmusicKey = pSkey;
    if (result.cookie) {
      const m = result.cookie.match(/\bqqmusic_key=([^;]+)/);
      if (m) qqmusicKey = m[1];
    }
    return {
      userId: uin,
...
function shouldBypassAllowlist(action, pathname) {
  ...
  if (pathname === '/qr/qq' || pathname === '/qr/netease') return true;
  if (action === 'health' || action === 'admin-status' || action === 'admin-toggle' || action === 'qr_bind' || action === 'manual_bind') return true;
  return false;
}
...
async function handleApi(req, res, reqUrl) {
  ...
  // 网易云手机号密码登录 (v0.24.0: 替代扫码登录)
  // 支持 POST JSON / POST form / GET query 三种方式
  // 成功后返回 6 位绑定码, 玩家回游戏输入绑定码完成绑定
  if (action === 'phone_login') {
    let phone = reqUrl.searchParams.get('phone') || '';
    let password = reqUrl.searchParams.get('password') || '';
    let countrycode = reqUrl.searchParams.get('countrycode') || '86';
    if (req.method === 'POST') {
      const raw = await readBody(req);
      const type = String(req.headers['content-type'] || '');
      if (type.includes('application/json')) {
        try {
          ...
          countrycode = countrycode || body.countrycode || '86';
        } catch {}
      } else {
        const params = new URLSearchParams(raw);
        phone = phone || params.get('phone') || '';
        password = password || params.get('password') || '';
        countrycode = countrycode || params.get('countrycode') || '86';
      }
    }
    if (!phone || !password) {
      return json(res, 400, { ok: false, error: 'missing_params', message: '请填写手机号和密码' });
    }
    const realIP = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim() || '';
    const result = await nc.phoneLogin(phone, password, countrycode, realIP);
    if (result.ok) {
      const info = buildLoginInfo('netease', { cookie: result.cookie, nickname: result.nickname, userId: result.userId });
      const bindCode = storeBindCode('netease', info);
      return json(res, 200, {
        ok: true,
        bindCode,
        nickname: result.nickname,
        message: '登录成功, 请回游戏输入绑定码',
      });
    }
    return json(res, 200, { ok: false, message: result.message || '登录失败' });
  }

  // 网易云手机号验证码登录 (v0.26.0: 替代密码登录, 用户体验更好)
  // 两步流程: nc_sendcode (发送验证码) → nc_verify (验证并获取绑定码)
  // 成功后返回 6 位绑定码, 玩家回游戏输入绑定码完成绑定
  if (action === 'nc_sendcode') {
    let phone = reqUrl.searchParams.get('phone') || '';
    let countrycode = reqUrl.searchParams.get('countrycode') || '86';
    if (req.method === 'POST') {
      const raw = await readBody(req);
    ...
    if (!phone) {
      return json(res, 400, { ok: false, error: 'missing_params', message: '请填写手机号' });
    }
    const realIP = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim() || '';
    const result = await nc.sendCaptcha(phone, countrycode, realIP);
    return json(res, 200, result.ok ? { ok: true, message: result.message } : { ok: false, message: result.message || '发送失败' });
  }

  if (action === 'nc_verify') {
    let phone = reqUrl.searchParams.get('phone') || '';
    let captcha = reqUrl.searchParams.get('captcha') || '';
    let countrycode = reqUrl.searchParams.get('countrycode') || '86';
    if (req.method === 'POST') {
      const raw = await readBody(req);
      const type = String(req.headers['content-type'] || '');
      if (type.includes('application/json')) {
        try {
          const body = JSON.parse(raw);
          phone = phone || body.phone || '';
          captcha = captcha || body.captcha || '';
          countrycode = countrycode || body.countrycode || '86';
        } catch {}
      } else {
        const params = new URLSearchParams(raw);
        phone = phone || params.get('phone') || '';
        captcha = captcha || params.get('captcha') || '';
        countrycode = countrycode || params.get('countrycode') || '86';
      }
    }
    if (!phone || !captcha) {
      return json(res, 400, { ok: false, error: 'missing_params', message: '请填写手机号和验证码' });
    }
    const realIP = String(req.headers['x-forwarded-for'] || '').split(',')[0].trim() || '';
    const result = await nc.verifyCaptcha(phone, captcha, countrycode, realIP);
    if (result.ok) {
      const info = buildLoginInfo('netease', { cookie: result.cookie, nickname: result.nickname, userId: result.userId });
      const bindCode = storeBindCode('netease', info);
      return json(res, 200, {
    ...
    return json(res, 200, { ok: false, message: result.message || '验证失败' });
  }
  ...
  if (action === 'qr_create') {
    ...
    // v0.23.0: 仅 QQ 支持扫码登录 (酷狗/酷我 VIP 无法在服务器获取, 已移除)
    if (server === 'qq') {
      const p = providers.get('qq');
      if (!p || !p.qrLoginStart) {
        return json(res, 400, { ok: false, error: 'qq_qr_not_supported', message: 'QQ 扫码登录不可用' });
      }
  ...
  if (action === 'qr_check') {
    ...
    if (server === 'netease') {
      ...
      // v0.24.1: 登录成功时生成 6 位绑定码, 玩家回游戏输入绑定码完成绑定
      if (result.status === 'ok') {
        const info = buildLoginInfo('netease', result);
        result.bindCode = storeBindCode('netease', info);
        result.message = result.message || '登录成功, 请回游戏输入绑定码';
      }
  ...
  // ==================== 手动绑定端点 (QQ Cookie 登录) ====================
  // 玩家在网页手动输入 cookie, 服务端生成绑定码
  // 参数: platform, cookie, nickname
  // v0.23.0: 仅支持 qq (酷狗/酷我 VIP 无法在服务器获取, 改用网易云兜底)
  if (action === 'manual_bind') {
    const platform = (reqUrl.searchParams.get('platform') || '').trim();
    const userId = (reqUrl.searchParams.get('userId') || '').trim();
    const token = (reqUrl.searchParams.get('token') || '').trim();
    const cookie = (reqUrl.searchParams.get('cookie') || '').trim();
    const nickname = (reqUrl.searchParams.get('nickname') || '').trim();

    if (!['qq', 'netease'].includes(platform)) {
      return json(res, 400, { ok: false, error: 'invalid_platform', message: '不支持的平台 (仅支持 qq/netease)' });
    }
    if (!cookie && !userId && !token) {
      return json(res, 400, { ok: false, error: 'missing_fields', message: '请至少填写 cookie 或 userId+token' });
    }

    const info = buildLoginInfo(platform, { userId, token, cookie, nickname });
    // 如果没有 nickname, 用默认值
    if (!info.nickname) {
      info.nickname = nickname || `${platform}用户`;
    }
    // 如果 buildLoginInfo 没有生成 cookie, 用用户输入的
    if (!info.cookie && cookie) info.cookie = cookie;
    if (!info.userId && userId) info.userId = userId;
    if (!info.token && token) info.token = token;

    const code = storeBindCode(platform, info);
    return json(res, 200, { ok: true, bindCode: code, message: '手动绑定成功, 请回游戏输入绑定码' });
  }
...
async function handle(req, res) {
  ...
  if (reqUrl.pathname === '/' || reqUrl.pathname === '/test') return staticFile('index.html', res);
