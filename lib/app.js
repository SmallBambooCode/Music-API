...
async function handleApi(req, res, reqUrl) {
  ...
  // 音频代理: 检测需要 Referer 的域名 (B站/kuwo/kugou/migu), 包装为代理 URL
  // 解灰源 (kuwo/kugou/migu) 有防盗链, 不代理会返回"暂时无法播放"TTS语音
  // Vercel 上也使用代理 (解灰源必须代理, 4.5MB限制对128kbps够用)
  ...
  try {
    ...
    if (type === 'song_full') {
      let detail = await api.songFull(id, userId, token);
      // QQ 解灰失败时, 用网易云搜索同名歌曲作为 fallback (QQ vkey 接口版权限制严格)
      if (!detail.ok && server === 'qq' && detail.name && detail.name !== '未知') {
        try {
          const neteaseApi = providers.get('netease');
          if (neteaseApi) {
            const keyword = detail.singer && detail.singer !== '未知'
              ? `${detail.name} ${detail.singer}` : detail.name;
            const searchResults = await neteaseApi.search(keyword, 1);
            if (searchResults && searchResults.length > 0) {
              const neteaseDetail = await neteaseApi.songFull(searchResults[0].id, '', '');
              if (neteaseDetail && neteaseDetail.ok && neteaseDetail.url) {
                detail = {
                  id: detail.id,
                  name: detail.name,
                  singer: detail.singer,
                  url: neteaseDetail.url,
                  lyric: neteaseDetail.lyric || detail.lyric,
                  time: neteaseDetail.time || detail.time,
                  ok: true,
      ...
      // 音频 URL 代理 (B站/解灰源需要 Referer)
      if (detail.url) detail.url = wrapProxyUrl(detail.url);
    ...
    if (type === 'url') {
      ...
      // 音频 URL 代理 (B站/解灰源需要 Referer)
      const finalUrl = wrapProxyUrl(result.url);
...
// 支持多平台域名 (解灰源防盗链需要正确 Referer)
// 域名匹配规则: hostname 包含关键词即匹配
const PROXY_DOMAIN_CONFIGS = [
  // B站 (DASH 音频流)
  { match: /bilivideo|akamaized/i, referer: 'https://www.bilibili.com/' },
  // kuwo (QQ解灰源, 防盗链返回"暂时无法播放"TTS语音)
