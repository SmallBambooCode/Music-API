...
function shouldBypassAllowlist(action, pathname) {
  ...
  if (action === 'health' || action === 'admin-status' || action === 'admin-toggle' || action === 'qr_bind' || action === 'manual_bind' || action === 'kw_captcha' || action === 'kw_send_sms' || action === 'kw_login') return true;
  return false;
}
...
async function handleApi(req, res, reqUrl) {
  ...
  // ==================== 酷我手机短信验证码登录 (三步流程) ====================
  // 步骤1: kw_captcha — 获取图片验证码
  //   返回: { ok, sessionId, img }
  // 步骤2: kw_send_sms — 发送短信验证码
  //   参数: sessionId, mobile, verifyCode (图片验证码)
  //   返回: { ok, message }
  // 步骤3: kw_login — 短信验证码登录
  //   参数: sessionId, smsCode (短信验证码)
  //   返回: { ok, bindCode, nickname }
  if (action === 'kw_captcha') {
    const kuwoProvider = providers.get('kuwo');
    if (!kuwoProvider || !kuwoProvider.getCaptcha) {
      return json(res, 400, { ok: false, message: '酷我验证码不可用' });
    }
    const result = await kuwoProvider.getCaptcha();
    return json(res, result.ok ? 200 : 502, result);
  }

  if (action === 'kw_send_sms') {
    const sessionId = (reqUrl.searchParams.get('sessionId') || '').trim();
    const mobile = (reqUrl.searchParams.get('mobile') || '').trim();
    const verifyCode = (reqUrl.searchParams.get('verifyCode') || '').trim();
    if (!sessionId || !mobile || !verifyCode) {
      return json(res, 400, { ok: false, message: '请填写 sessionId, mobile, verifyCode' });
    }
    const kuwoProvider = providers.get('kuwo');
    const result = await kuwoProvider.sendSms(sessionId, mobile, verifyCode);
    return json(res, 200, result);
  }

  if (action === 'kw_login') {
    const sessionId = (reqUrl.searchParams.get('sessionId') || '').trim();
    const smsCode = (reqUrl.searchParams.get('smsCode') || '').trim();
    if (!sessionId || !smsCode) {
      return json(res, 400, { ok: false, message: '请填写 sessionId, smsCode' });
    }
    const kuwoProvider = providers.get('kuwo');
    const result = await kuwoProvider.loginByMobile(sessionId, smsCode);
    if (!result.ok) {
      return json(res, 200, { ok: false, message: result.message || '酷我登录失败' });
    }
    const info = buildLoginInfo('kuwo', result);
    const code = storeBindCode('kuwo', info);
    return json(res, 200, { ok: true, bindCode: code, nickname: info.nickname, message: '酷我登录成功, 请回游戏输入绑定码' });
  }

  // ==================== 手动绑定端点 (备用方式) ====================
  // 玩家在网页手动输入 cookie 字段值, 服务端生成绑定码
  // 参数: platform, userId, token, cookie, nickname
  if (action === 'manual_bind') {
    ...
    if (!['qq', 'kugou', 'kuwo'].includes(platform)) {
      return json(res, 400, { ok: false, error: 'invalid_platform', message: '不支持的平台' });
    }
