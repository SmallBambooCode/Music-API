...
function shouldBypassAllowlist(action, pathname) {
  ...
  if (pathname === '/qr/qq' || pathname === '/qr/netease') return true; // 二维码图片不需鉴权
  if (action === 'health' || action === 'admin-status' || action === 'admin-toggle') return true;
  // QR 登录端点也需要白名单 (避免陌生人消耗登录配额)
  return false;
}

// 二维码图片端点 (返回 PNG, 不需要白名单但需要有效 token)
async function handleQrImage(req, res, reqUrl, platform) {
  ...
  if (platform === 'qq') {
    const buf = qqProvider ? qqProvider.getQrImage(token) : null;
    if (!buf) {
...
async function handleApi(req, res, reqUrl) {
  ...
  if (action === 'qr_create') {
    ...
    if (server === 'qq') {
      const qqProvider = providers.get('qq');
      if (!qqProvider || !qqProvider.qrLoginStart) {
        return json(res, 400, { ok: false, error: 'qq_qr_not_supported', message: 'QQ 扫码登录不可用' });
      }
      const result = await qqProvider.qrLoginStart();
    ...
    if (server === 'netease') {
      ...
      const result = await nc.qrLoginStart();
      const proxyBase = `${req.headers['x-forwarded-proto'] || 'http'}://${req.headers['x-forwarded-host'] || req.headers.host || `127.0.0.1:${nc.getAdapterPort()}`}`;
      return json(res, result.ok ? 200 : 502, {
        ok: result.ok,
        server: 'netease',
  ...
  if (action === 'qr_check') {
    ...
    if (server === 'qq') {
      const qqProvider = providers.get('qq');
      if (!qqProvider || !qqProvider.qrLoginCheck) {
        return json(res, 400, { ok: false, error: 'qq_qr_not_supported', message: 'QQ 扫码登录不可用' });
      }
      const result = await qqProvider.qrLoginCheck(token);
      return json(res, 200, { ok: true, server: 'qq', ...result });
    }
    if (server === 'netease') {
      if (!nc.qrLoginCheck) {
        return json(res, 400, { ok: false, error: 'netease_qr_not_supported', message: '网易云扫码登录不可用' });
  ...
  // 用户鉴权信息:
  //   - QQ: userId=uin (QQ号), token=p_skey
  //   - 网易云: cookie=MUSIC_U=xxx; __csrf=xxx (per-player, 优先于 NCM_COOKIE)
  //   - 酷狗/酷我: userid/token (原有逻辑)
  ...
  try {
    ...
    if (type === 'song_full') {
      ...
      if (server === 'qq') {
        ...
        detail = await api.songFull(id, userId, token);
      } else if (server === 'netease') {
        // 网易云: cookie 透传 (per-player)
        detail = await api.songFull(id, cookie);
      } else {
        // 酷狗/酷我: 原有 userid+token
        detail = await api.songFull(id, userId, token);
      }
    ...
    if (type === 'url') {
      let result;
      if (server === 'qq') {
        result = await api.songUrl(id, userId, token);
      } else if (server === 'netease') {
        result = await api.songUrl(id, cookie);
      } else {
        result = await api.songUrl(id, userId, token);
      }
      ...
      if (!result.ok) {
        return json(res, 502, {
          ...
          message: server === 'netease' ? '网易云未返回可播放 URL (可能是 VIP, 请扫码登录)' :
                   server === 'qq' ? 'QQ 音乐未返回可播放 URL (可能是 VIP, 请扫码登录)' :
