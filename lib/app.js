...
// 多平台 API 路由层 v0.14.0
// 支持: netease (enhanced) / kugou / kuwo
// 端点:
//   /api?action=health                       — 健康检查
//   /api?action=admin-status                 — 管理员查询
//   /api?action=admin-toggle                 — 临时关闭/开启白名单
//   /api?action=probe                        — 网易云底层诊断
//   /api?action=enhanced&path=...            — 网易云 enhanced 模式 (仅 netease)
...
async function handleApi(req, res, reqUrl) {
  ...
  try {
    ...
    if (type === 'url') {
      ...
      if (wantsJson) return json(res, 200, { ok: true, url: result.url, server });
    ...
    if (type === 'lrc' || type === 'lyric') {
      ...
      const text = lrc && typeof lrc === 'object' ? lrc.lyric : lrc;
      return text(res, 200, text || '');
    }
