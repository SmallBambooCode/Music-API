...
async function call(name, params = {}, customCookie = '') {
  const fn = api[name];
...
// 返回 { ok, token, qrUrl } — qrUrl 指向 /qr/netease?token=xxx (返回 base64 图片)
async function qrLoginStart() {
  // 1. 获取 unikey
  const keyResp = await call('login_qr_key', {});
  ...
  const createResp = await call('login_qr_create', { key: unikey, qrimg: true });
  ...
  QR_SESSIONS.set(token, { key: unikey, qrimgBase64, createdAt: Date.now() });
...
async function qrLoginCheck(token) {
  ...
  const checkResp = await call('login_qr_check', { key: sess.key });
