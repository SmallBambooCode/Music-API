...
function sanitizeParams(paramsObj = {}) {
  const out = {};
...
async function enhancedRaw(pathname, query = {}) {
  const fn = pathToFunction(pathname);
  if (!fn) {
    return {
      ok: false,
      status: 404,
      ...
      body: {
        ok: false,
        error: 'unsupported_enhanced_path_in_module_mode',
        message: 'module 模式只开放测试页里的安全映射路由。',
        path: pathname,
      },
    };
  }

  const p = sanitizeParams(query);

  if (fn === 'song_url_v1' && !p.level) p.level = process.env.NCM_LEVEL || 'standard';
  if (fn === 'song_url' && !p.br) p.br = '999000';
  if (fn === 'search' && !p.type) p.type = '1';
  if ((fn === 'search' || fn === 'cloudsearch') && !p.limit) p.limit = '10';
  if ((fn === 'playlist_track_all' || fn === 'artist_songs') && !p.limit) p.limit = '10';
  if ((fn === 'playlist_track_all' || fn === 'artist_songs' || fn === 'search' || fn === 'cloudsearch') && !p.offset) p.offset = '0';

  try {
    const body = await call(fn, p);
    return {
      ok: true,
      status: 200,
      url: `module:${fn}`,
