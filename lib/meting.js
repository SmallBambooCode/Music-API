  ...
  if (!url) {
    return {
      ...
      meta: {
        ...
        level,
        reason: 'No playable URL returned by upstream for the configured account/session.',
      },
    };
  }

  return { url, meta: { level, br: item.br, size: item.size, type: item.type, fee: item.fee } };
}
