...
function coverOf(song) {
  return (song.al && (song.al.picUrl || song.al.pic_str)) ||
    (song.album && (song.album.picUrl || song.album.pic)) ||
    (song.picUrl || song.cover || '');
}

function mapSong(song) {
  const id = String(song.id || song.songId || song.resourceId || '');
  return {
...
function pickSongsFromBody(body) {
  ...
  if (body.album && Array.isArray(body.album.songs)) return body.album.songs;
  return [];
}
...
function pickUrlItem(body) {
  ...
  if (Array.isArray(body.urls)) return body.urls[0] || {};
...
async function enhancedUrl(id, query = {}) {
  ...
  for (const level of levels) {
    try {
      const res = await call('/song/url/v1', { id, level, timestamp: Date.now() }, { ttl: urlTtl() });
      const body = res.body || {};
      const item = pickUrlItem(body);
      const url = normalizeUrl(item.url);
      attempts.push({ route: '/song/url/v1', level, code: body.code || item.code, fee: item.fee, type: item.type, br: item.br, hasUrl: Boolean(url) });
  ...
  try {
    const res = await call('/song/url', { id, br: 999000, timestamp: Date.now() }, { ttl: urlTtl() });
    const body = res.body || {};
    const item = pickUrlItem(body);
    const url = normalizeUrl(item.url);
    attempts.push({ route: '/song/url', code: body.code || item.code, fee: item.fee, type: item.type, br: item.br, hasUrl: Boolean(url) });
    ...
    attempts.push({ route: '/song/url', error: error.message });
  }

  return { url: '', meta: { provider: 'api-enhanced', reason: 'No playable URL returned by enhanced upstream.', attempts } };
}

async function getUrl(id, query = {}) {
  const enhanced = await enhancedUrl(id, query);
  if (enhanced.url) return enhanced;

  // This mirrors xizeyoupan/Meting-API behavior: when the eapi URL is empty,
  // return NetEase's public outer URL instead of returning an empty result.
  const fallback = outerUrl(id);
  return {
    url: fallback,
    meta: {
      provider: 'netease-outer-url',
      route: 'https://music.163.com/song/media/outer/url',
      reason: 'Enhanced upstream returned no URL; using Meting-API compatible outer URL fallback.',
      previous: enhanced.meta,
    },
  };
}
...
async function handleMeting(req, query) {
  ...
  if (type === 'url') {
    const result = await getUrl(id, query);
    if (query.raw === '1' || query.json === '1' || query.format === 'json' || query.debug === '1') return { kind: 'json', status: 200, data: result };
...
module.exports = { handleMeting, fillMetingLinks, mergeLyric, getUrl, outerUrl };
