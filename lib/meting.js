...
async function getLyric(id) {
  const res = await call('/lyric', { id, tv: -1, lv: -1, rv: -1, kv: -1 }, { ttl: jsonTtl() });
  const body = res.body || {};
...
function urlProviderMode() {
  return String(process.env.URL_PROVIDER || 'enhanced-then-fallback').trim().toLowerCase();
}

function shouldUseFallbackFirst() {
  const mode = urlProviderMode();
  return mode === 'fallback-first' || mode === 'meting-first' || mode === 'old-first';
}

function shouldUseFallbackOnly() {
  const mode = urlProviderMode();
  return mode === 'fallback-only' || mode === 'meting-only' || mode === 'old-only';
}
...
async function fallbackMetingUrl(req, id, query = {}) {
  ...
  const response = await fetch(url.toString(), {
    ...
    redirect: 'manual',
    headers: {
      'User-Agent': 'meting-enhanced-vercel/0.5 fallback-url',
      Referer: req && (req.headers.referer || req.headers.referrer) || '',
  ...
  if (location && /^https?:\/\//i.test(location)) {
    return { url: location.replace(/^http:\/\//, 'https://'), meta: { provider: 'meting-fallback', status: response.status } };
  }
  ...
  if (Array.isArray(body)) {
    for (const item of body) candidates.push(item && item.url, item && item.data && item.data.url);
  }
  if (body && typeof body === 'object') {
    candidates.push(
      ...
      body.data && body.data[0] && body.data[0].url,
      body.data && body.data[0] && body.data[0].src
    );
  }
...
async function enhancedUrl(id, query = {}) {
  ...
  // Some NCM API builds expose /song/url but not /song/url/v1. Try it as a final fallback.
  try {
  ...
  return {
    url: '',
    meta: {
      provider: 'api-enhanced',
      reason: 'No playable URL returned by enhanced upstream.',
      attempts,
    },
  };
}
...
async function getUrl(id, query = {}, req = null) {
  if (req && (shouldUseFallbackFirst() || shouldUseFallbackOnly())) {
    ...
    if (shouldUseFallbackOnly()) {
      return { url: '', meta: { reason: 'Fallback API did not return a playable URL.', fallbackConfigured: Boolean(getFallbackApiBase()), fallbackError: fallbackFirst && fallbackFirst.error } };
    }
  }
